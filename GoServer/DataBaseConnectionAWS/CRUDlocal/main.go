...
func main() {
	db, err = sql.Open("mysql", "awsuser:mypassword@tcp(ydbinstance.ckmemwolszx6.us-east-2.rds.amazonaws.com:3306)/mydb?charset=utf8")
	check(err)
	defer db.Close()
...
func amigos(w http.ResponseWriter, req *http.Request) {
	rows, err := db.Query(`SELECT nameEmployee FROM Employee;`)
	check(err)
	defer rows.Close()
